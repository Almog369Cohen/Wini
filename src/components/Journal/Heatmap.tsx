...
export default function Heatmap({ entries, days = 90 }: HeatmapProps) {
  ...
  const getColor = (day: { falls: number; victories: number; total: number; key: string }) => {
    ...
    if (day.total === 0) return isDark ? '#2a2a2d' : '#dceee9';
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-center gap-1 mt-3">
        ...
        <div className="w-3 h-3 rounded-[2px] bg-coral" />
        <div className="w-3 h-3 rounded-[2px] bg-sea" />
        <div className="w-3 h-3 rounded-[2px] bg-sand-light" />
        <div className="w-3 h-3 rounded-[2px] bg-sea" />
        <div className="w-3 h-3 rounded-[2px] bg-sage" />
        <span className="text-[9px] text-text-light">ניצחונות</span>
        <div className="w-px h-3 bg-cream-dark mx-1" />
        <div className="w-3 h-3 rounded-[2px] bg-cream-dark" />
